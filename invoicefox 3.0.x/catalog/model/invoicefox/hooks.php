...
function opencart_invfox__trace($x) { error_log(print_r($x,true)." **** INVFOXOC ****"); } // todo -- move to class
...
class ModelInvoiceFoxHooks extends Model { 
  ...
  public function changeOrderStatusHook($status_id, $comment, $order_id) {
    ...
    opencart_invfox__trace("order status ; conf create status ; invoicefox_id");
    opencart_invfox__trace($data['order_status']);
    opencart_invfox__trace($this->CONF['create_invfox_document_on_status']);
	$invoicefox_id = $this->model_invoicefox_order->getInvoicefoxId($order_id); 
    opencart_invfox__trace($invoicefox_id);
	...
	if ($data['order_status'] == $this->CONF['close_invfox_document_on_status']) {
      ...
      $invoice_no = $order_id ;
	  $r = $api->markInvoicePaid($invoice_no);

      opencart_invfox__trace($r);

      if (true) { // TODO -- check the response
          $newComment .= ($newComment ? "\n" : "") . "- Payment was added to {$this->CONF['APP_NAME']}.";
      }
  ...
  private function makeInvoiceFromOrder($order_id) {
	...
	if ($this->CONF['document_to_make'] != 'inventory') {
	  ...
	  $invoice_no = $order_id ; // $order['invoice_prefix'] . $order['order_id'] ;
	...
	if ($r2->isOk()) {    
	  ...
	  $comment .= "- Invoice based on order {$invoice_no} was created at {$this->CONF['APP_NAME']}.";
	} 
	...
	if ($r2->isOk()) {    
	  $invA = $r2->getData();
	  $this->model_invoicefox_order->insertInvoicefoxId($order_id,$invA[0]['id']);
	  $comment .= "- pro forma Račun št.: {$invoice_no} was created at {$this->CONF['APP_NAME']}.";
	} 
	...
	if ($r2->isOk()) {    
	  $invA = $r2->getData();
	  $this->model_invoicefox_order->insertInvoicefoxId($order_id,$invA[0]['id']);
	  $comment .= "Inventory sales document No. {$invoice_no} was created at {$this->CONF['APP_NAME']}.";
	} 
      }
