...
class ControllerExtensionModuleInvoicefox extends Controller {
	...
	public function index() { 
		...
		$this->load->language('extension/module/invoicefox');
		...
		if (isset($this->error['create_invfox_document_on_status'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/invoicefox', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('extension/module/invoicefox', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		*/
		...
		$this->response->setOutput($this->load->view('extension/module/invoicefox', $data));
